...
Footer links at the bottom of the controls open popups:

- **Privacy** — the app's privacy policy, bundled at
  `assets/legal/privacy_policy.txt` and shown in-app. Classic Lines collects **no
  data** (a "zero-collection" policy), which Play requires to be referenced in
  the app. The canonical copy lives at
  <https://github.com/amuletgrey/PrivacyPolicies>; the Play Console still needs
  that URL entered separately.
- **Help** — how to play and the controls.
- **About** — description, developer/contact, and version (text in `lib/info.dart`).
...
- **Name:** Classic Lines. **Application ID:** `com.maxcavrilon.classiclines`.
  **Developer:** Max Cavrilon (`maxcavrilon@gmail.com`).
